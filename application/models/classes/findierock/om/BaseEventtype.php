  ...
  const PEER = 'EventtypePeer';
	...
	public function setEventtypeid($v)
	{
		...
		if ($this->eventtypeid !== $v) {
			$this->eventtypeid = $v;
			$this->modifiedColumns[] = EventtypePeer::EVENTTYPEID;
		}
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = EventtypePeer::TYPE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->eventtypeid = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(EventtypePeer::EVENTTYPEID) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.EventtypePeer::EVENTTYPEID.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setEventtypeid($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setEventtypeid($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EventtypePeer::EVENTTYPEID)) $criteria->add(EventtypePeer::EVENTTYPEID, $this->eventtypeid);
		if ($this->isColumnModified(EventtypePeer::TYPE)) $criteria->add(EventtypePeer::TYPE, $this->type);
	...
	{
		$criteria = new Criteria(EventtypePeer::DATABASE_NAME);
		$criteria->add(EventtypePeer::EVENTTYPEID, $this->eventtypeid);
	...
	/**
	 * Generic method to set the primary key (eventtypeid column).
	 *

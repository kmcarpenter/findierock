...
abstract class BaseMember extends BaseObject  implements Persistent
{
  ...
  const PEER = 'MemberPeer';
	...
	/**
	 * The value for the screenname field.
	 * @var        string
	 */
	...
	/**
	 * Get the [screenname] column value.
	 * 
	 * @return     string
	 */
	...
	public function setMemberid($v)
	{
		...
		if ($this->memberid !== $v) {
			$this->memberid = $v;
			$this->modifiedColumns[] = MemberPeer::MEMBERID;
		}
	...
	/**
	 * Set the value of [screenname] column.
	 * 
	...
	public function setScreenname($v)
	{
		...
		if ($this->screenname !== $v) {
			$this->screenname = $v;
			$this->modifiedColumns[] = MemberPeer::SCREENNAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->screenname = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = MemberPeer::NUM_COLUMNS - MemberPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(MemberPeer::MEMBERID) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.MemberPeer::MEMBERID.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setMemberid($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MemberPeer::MEMBERID)) $criteria->add(MemberPeer::MEMBERID, $this->memberid);
		if ($this->isColumnModified(MemberPeer::SCREENNAME)) $criteria->add(MemberPeer::SCREENNAME, $this->screenname);
	...
	{
		$criteria = new Criteria(MemberPeer::DATABASE_NAME);
		$criteria->add(MemberPeer::MEMBERID, $this->memberid);
	...
	/**
	 * Generic method to set the primary key (memberid column).
	 *
	...
	{
		$copyObj->setScreenname($this->screenname);

		$copyObj->setNew(true);
		$copyObj->setMemberid(NULL); // this is a auto-increment column, so set to default value
	}
